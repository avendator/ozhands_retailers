...
(function ( $, window ) {
	...
	productImportForm.prototype.run_import = function() {
		...
		$.ajax( {
			...
			data: {
				action          : 'ozh_do_ajax_product_import',
			...
			success: function( response ) {
				...
				if ( response.success ) {
					...
					window.location = response.data.url + '/?percentage=' + parseInt( $this.percentage, 10 ) + '&products-imported=' + parseInt( $this.imported, 10 ) + '&products-failed=' + parseInt( $this.failed, 10 ) + '&products-updated=' + parseInt( $this.updated, 10 ) + '&products-skipped=' + parseInt( $this.skipped, 10 );
				}
			}
